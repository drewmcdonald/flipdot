...
import { renderTransition, createBlankFrame } from "./patterns/render.ts";
...
app.post("/api/flipdot/pattern", bearerAuthMiddleware, async (c) => {
  try {
    ...
    // Valid pattern types
    const validTypes = [
      "wave", "rain", "spiral", "checkerboard", "random", "expand",
      "gameoflife", "matrix", "sparkle", "pulse", "scan", "fire", "snake"
    ];
    ...
    const source = patternStorage.set(config, { priority, ttl_ms, interruptible });
    router.registerSource(source);
...
app.get("/api/flipdot/patterns/list", bearerAuthMiddleware, (c) => {
  const patterns = [
    { type: "wave", description: "Horizontal or vertical sine wave", options: { vertical: "boolean", amplitude: "number", frequency: "number", speed: "number" } },
    { type: "rain", description: "Falling dots", options: { density: "number (0-1)", speed: "number" } },
    { type: "spiral", description: "Spiral from center", options: { speed: "number", arms: "number" } },
    { type: "checkerboard", description: "Animated checkerboard", options: { size: "number" } },
    { type: "random", description: "Random noise", options: { density: "number (0-1)" } },
    { type: "expand", description: "Expanding circles or squares", options: { speed: "number", shape: "'circle' | 'square'" } },
    { type: "gameoflife", description: "Conway's Game of Life", options: { density: "number (0-1)", seed: "number" } },
    { type: "matrix", description: "Matrix-style falling characters", options: { density: "number (0-1)", speed: "number" } },
    { type: "sparkle", description: "Random sparkles", options: { density: "number (0-1)" } },
    { type: "pulse", description: "Pulsing effect from center", options: { speed: "number" } },
    { type: "scan", description: "Scanner effect (back and forth)", options: { vertical: "boolean", speed: "number" } },
    { type: "fire", description: "Fire effect from bottom", options: { intensity: "number (0-1)" } },
    { type: "snake", description: "Snake/worm moving around", options: { length: "number", speed: "number" } },
  ];
...
app.get("/api/flipdot/transitions/list", bearerAuthMiddleware, (c) => {
  const transitions = [
    { type: "wipe", description: "Wipe from one direction", options: { direction: "'left' | 'right' | 'up' | 'down'" } },
    { type: "fade", description: "Dithered fade for binary display" },
    { type: "dissolve", description: "Random pixel dissolve", options: { seed: "number" } },
    { type: "slide", description: "Slide in from direction", options: { direction: "'left' | 'right' | 'up' | 'down'" } },
    { type: "checkerboard", description: "Checkerboard pattern reveal", options: { size: "number" } },
    { type: "blinds", description: "Venetian blinds effect", options: { vertical: "boolean", blindSize: "number" } },
...
app.post("/api/flipdot/transition", bearerAuthMiddleware, async (c) => {
  try {
    ...
    const validTypes = [
      "wipe", "fade", "dissolve", "slide", "checkerboard",
      "blinds", "center_out", "corners", "spiral"
    ];
...
app.post("/api/flipdot/playlist", bearerAuthMiddleware, async (c) => {
  try {
    ...
    if (body.items.length > 50) {
      return c.json({ error: "Playlist cannot contain more than 50 items" }, 400);
    }

    // Clear existing custom content (optionally keep clock)
    const keepClock = body.keep_clock !== false;
    const sources = router.getSources();
    const customSources = sources.filter((s) => keepClock ? s.type !== "clock" : true);
...
app.get("/", async (c) => {
  try {
    ...
    return c.json({
      ...
      version: "2.2",
      endpoints: [
        { path: "/api/flipdot/content", method: "GET", auth: "required", description: "Get current content playlist" },
        { path: "/api/flipdot/text", method: "POST", auth: "required", description: "Submit text message" },
        { path: "/api/flipdot/clear", method: "POST", auth: "required", description: "Clear all custom content" },
        { path: "/api/flipdot/pattern", method: "POST", auth: "required", description: "Submit pattern animation" },
        { path: "/api/flipdot/pattern/clear", method: "POST", auth: "required", description: "Clear all patterns" },
        { path: "/api/flipdot/patterns/list", method: "GET", auth: "required", description: "List available patterns" },
        { path: "/api/flipdot/transition", method: "POST", auth: "required", description: "Create transition animation" },
        { path: "/api/flipdot/transitions/list", method: "GET", auth: "required", description: "List available transitions" },
        { path: "/api/flipdot/playlist", method: "POST", auth: "required", description: "Submit playlist of items" },
        { path: "/health", method: "GET", auth: "none", description: "Health check" },
      ],
    });
  }
});
